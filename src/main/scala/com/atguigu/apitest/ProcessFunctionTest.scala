...
object ProcessFunctionTest {
  def main(args: Array[String]): Unit = {
    ...
    val processedStream: DataStream[String] = dataStream.keyBy(_.id).process(new TempIncreAlert())
    val processedStream1: DataStream[String] = dataStream.keyBy(_.id).process(new TempChargeAlert2(10.0)) //方法一
    val processedStream2: DataStream[(String, Double, Double)] = dataStream.keyBy(_.id).flatMap(new TempChargeAlert(10.0)) //方法二
    val processedStream3: DataStream[(String, Double, Double)] = dataStream.keyBy(_.id)
      .flatMapWithState[(String, Double, Double), Double] { //方法三
        ...
        //如果有状态，就应该与 上次的温度值比较差值，如果大于阈值就输出报警
    ...
    //processedStream.print("stream processed data")
    //processedStream1.print("stream1 processed data")
    processedStream2.print("stream2 processed data")
    //processedStream3.print("stream3 processed data")
...
class TempIncreAlert() extends KeyedProcessFunction[String, SensorReading, String] {
  ...
  override def processElement(value: SensorReading, ctx: KeyedProcessFunction[String, SensorReading, String]#Context, out: Collector[String]): Unit = {
    ...
    //温度上升且没有设过定时器 则注册定时器
    if (value.temperature > preTemp && curTimeTs == 0) {
...
class TempChargeAlert2(threshold: Double) extends KeyedProcessFunction[String, SensorReading, (String, Double, Double)] {
  //定义一个状态变量，保存上次的温度值
  private val lastTempState: ValueState[Double] = getRuntimeContext.getState(new ValueStateDescriptor[Double]("lastTemp", classOf[Double]))
...
class TempChargeAlert(threshold: Double) extends RichFlatMapFunction[SensorReading, (String, Double, Double)] {
